...
import { handleForwardThunk } from '../../redux/reducers/gameAction.js'
...
const Cell = ({ id, color }) => {
        ...
        const piecesAtPosition = useMemo(() => plottedPieces.filter(item => item.pos == id),
                [plottedPieces, id],
        );
        ...
        return (
                <View
                        ...
                        {isStarSpot && <Ionicans name='star-outline' size={20} color="grey" />}
                        {isArrowSpot && (
                                <Ionicans name='arrow-forward-outline'
                                        style={{
                                                transform: [{
                                                        rotate:
                                                                id === 38 ? '180deg' :
